% ********************************************************************
    ...
    % ********************************************************************
                                                                         ...
                                                                         function[T, Q]
    = print_prob(A, dir_in) % % [ T, Q ] = print_prob(A[, dir]) % % % Given a sparse matrix,
                                       write out the permutations and factorization %
    % The files are created in the current local directory by default % If optional "dir" is given,
                                       the files are created under that directory
...
isok = (status == 1);
if (~isok)
    , error(sprintf('print_prob: mkdir for %s return msg=%s, msgid=%g', ... dir, msg, msgid));
return;
end;
end;
