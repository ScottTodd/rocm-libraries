...
% -- -- -- -- -- -- -- -- -- -- -- -- -- - % generate random sparse matrix % with nz non
    - zero elements % -- -- -- -- -- -- -- -- -- -- -- -- -- -per
    = nz / (n * n);
A = sprand(n, n, per);

% -- -- -- -- -- -- -- -- -- -- -- -- -- - % Take lower part L and make it diagonally dominant
    % (this means that the symmetric matrix L + L ^ T will be % positive definite)
    % -- -- -- -- -- -- -- -- -- -- -- -- -- -A
    = tril(A);
A = fixmat(A, n / 2);

% -- -- -- -- -- -- -- -- -- -- -- -- -- - % If necessary,
    remove extra non - zero elements % so that the final symmetric matrix has no more % than nz non
    - zero elements % -- -- -- -- -- -- -- -- -- -- -- -- -- -nzsym
    = (nz + n) / 2;
dif = nnz(A) - nzsym;
if (dif > 0)
...
% -- -- -- -- -- -- -- -- -- -- -- -- -- - % compute the final symmetric matrix
    % (functions work with only the lower part,
        but better % to have a full symmetric to generate the linear systems)
    % -- -- -- -- -- -- -- -- -- -- -- -- -- -A
    = A + transpose(A);
...
% -- -- -- -- -- -- -- -- -- -- -- -- -- - % Generate output files
    % -- -- -- -- -- -- -- -- -- -- -- -- -- -disp('') disp('Generating files...') isok
    = printall("po", A, T, Q);
if (isok)
    , disp('OK') disp('') else disp('FAILURE!');
disp('') end;

% -- -- -- -- -- -- -- -- -- -- -- -- % quick validation % -- -- -- -- -- -- -- -- -- -- -- --ptr
    = dlmread('ptrT');
ind = dlmread('indT');
val = dlmread('valT');
...
disp('Quick checking:') b = dlmread('B_1');
x = dlmread('X_1');
err1 = norm(q * t * t '*q' * x - b, "fro") b = dlmread('B_10');
x = dlmread('X_10');
err2 = norm(q * t * t '*q' * x - b, "fro") b = dlmread('B_30');
x = dlmread('X_30');
err3 = norm(q * t * t '*q' * x - b, "fro") disp('')
