...
def run_benchmarks(benchmark_context):
    ...
    for benchmark_name in benchmark_names:
        ...
        results_json_path = os.path.join(
            benchmark_context.benchmark_dir, results_json_name
        )
        args = [
            benchmark_path,
...
def create_benchmark_folder(benchmark_context, api_context):
    formatted_datetime = datetime.strftime(
        benchmark_context.run_datetime, "%Y%m%d_%H%M%S"
    )
    new_folder_name = (
        f"{formatted_datetime}_{benchmark_context.version}_{benchmark_context.gpu_name}"
    )
    create_folder_url = urljoin(
        api_context.endpoint, f"files/folder/{api_context.folder_id}"
    )
...
def upload_results(folder_id, api_context, paths_to_upload):
    ...
    for path in paths_to_upload:
        ...
        upload_file_request = urllib.request.Request(
            url=upload_file_url,
            data=body_bytes,
...
def main():
    ...
    api_context = ApiContext(
        args.api_endpoint, args.api_base_folder_id, args.api_auth_token
    )
