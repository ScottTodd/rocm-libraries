...
def parse_args():
    ...
    default_gpus = (
        "gfx906:xnack-,gfx1030,gfx1100,gfx1101,gfx1102,gfx1151,gfx1200,gfx1201"
    )
    ...
    parser.add_argument(
        "-a",
        "--architecture",
        dest="gpu_architecture",
        required=False,
        default=default_gpus,  #:sramecc+:xnack-" ) #gfx1030" ) #gfx906" ) # gfx1030" )
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="Do not apply offload compression (deafult: False)",
    )
...
def config_cmd():
    ...
    if OS_info["ID"] == "windows":
        ...
        cmake_platform_opts.append(
            f"-DWIN32=ON -DCPACK_PACKAGING_INSTALL_PREFIX="
        ...
        cmake_platform_opts.append(
            '-DCMAKE_CXX_FLAGS="-D_ENABLE_EXTENDED_ALIGNED_STORAGE"'
        )

        rocm_cmake_path = (
            '"' + cmake_path(os.getenv("ROCM_CMAKE_PATH", "C:/hipSDK")) + '"'
        )
        generator = f"-G Ninja"
    ...
    # packaging options
    cmake_pack_options = (
        f"-DCPACK_SET_DESTDIR=OFF -DCPACK_INCLUDE_TOPLEVEL_DIRECTORY=OFF"
    )
    ...
    if args.build_clients:
        cmake_options.append(
            f"-DBUILD_TEST=ON -DBUILD_BENCHMARK=ON -DBUILD_EXAMPLE=ON -DBUILD_DIR={build_dir}"
        )
