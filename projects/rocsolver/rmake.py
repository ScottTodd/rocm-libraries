...
def config_cmd():
    ...
    if os.name == "nt":
        ...
        if os.getenv("NO_VCPKG") is None:
            ...
            cmake_options.append(
                f"-DCMAKE_TOOLCHAIN_FILE={vcpkg_toolchain.as_posix()} -DVCPKG_TARGET_TRIPLET=x64-windows"
            )
    ...
    cmake_base_options = "-DROCM_PATH={0} -DCMAKE_PREFIX_PATH={1}".format(
        rocm_path, ";".join(cmake_prefix_path)
    )
    ...
    if args.build_clients:
        ...
        cmake_options.append(
            "-DBUILD_CLIENTS_TESTS=ON -DBUILD_CLIENTS_BENCHMARKS=ON -DBUILD_CLIENTS_SAMPLES=ON"
        )
