...
% -- -- -- -- -- -- -- -- -- -- -- -- -- - % If necessary,
    remove extra non - zero elements % -- -- -- -- -- -- -- -- -- -- -- -- -- -dif = nnz(A) - nz;
if (dif > 0)
    , A = cleanmat(A, dif);
end;
...
x = dlmread('X_1');
err1 = norm(p '*l*u*q' * x - b, "fro") b = dlmread('B_10');
x = dlmread('X_10');
err2 = norm(p '*l*u*q' * x - b, "fro") b = dlmread('B_30');
x = dlmread('X_30');
err3 = norm(p '*l*u*q' * x - b, "fro") disp('')
