...
def batch(script, xml):
    ...
    if os.path.isfile(rtest_cwd_path) and os.path.dirname(rtest_cwd_path).endswith(
        ...
        if test_dir == "":
            print(
                f"ERROR: Could not determine a valid test directory. Checked {', '.join(search_paths)}."
            )
            return 2
    fail = False
    for i in range(len(script)):
        ...
        if cmd.startswith("%XML%"):
            ...
            if len(fileversion) == 0:
                print(
                    "INFO: Could not find the version of this xml configuration file. Version 0.1 assumed."
                )
                ...
                if version > SCRIPT_VERSION:
                    print(
                        f"ERROR: This file requires script version >= {version}, have version {SCRIPT_VERSION}"
                    )
                    exit(1)
            ...
            for name, val in args.argument.items():
                var_subs[name] = val
            available_cases = xml.getElementsByTagName(
                "test"
            ) + xml.getElementsByTagName("emulation")
