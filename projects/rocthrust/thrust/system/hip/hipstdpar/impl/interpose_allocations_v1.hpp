...
__attribute__((weak)) void* __hipstdpar_hidden_memalign(::std::size_t, ::std::size_t);
#    if defined(_POSIX_MAPPED_FILES)
#      define __HIPSTDPAR_INTERPOSE_ALLOC_CAN_MMAP__
__attribute__((weak)) void* __hipstdpar_hidden_mmap(void*, ::std::size_t, int, int, int, ::off_t) noexcept;
__attribute__((weak)) int __hipstdpar_hidden_munmap(void*, ::std::size_t) noexcept;
...
class Stack_accessor final
{
  ...
  { // Due to how the kernel manages memory, we have to pre-access.
    ::std::uint64_t r{1};
    for (auto i = 0u; i != n_ / sizeof(*ps_); ++i)
    {
      r += ps_[i];
    }
    return r;
...
public:
  ...
  Stack_accessor()
  {
    ...
    {
      throw ::std::runtime_error("Failed to get thread stack attributes.");
    }
    if (!ps_ || n_ == 0)
    {
      return;
    ...
    {
      throw ::std::runtime_error("Failed to retrieve accelerator for HIPSTDPAR");
    }
    if (touch_stack_() && hipMemAdvise(ps_, n_, hipMemAdviseSetAccessedBy, d_) != hipSuccess)
    {
      throw ::std::runtime_error("Failed to make thread stack accessible.");
    }
  }
  ~Stack_accessor()
  {
    ...
    {
      ::std::cerr << "Failed to unset thread stack accessibility." << ::std::endl;
    }
  }
};
...
extern "C" {
inline __attribute__((used)) void* __hipstdpar_aligned_alloc(std::size_t a, std::size_t n)
{
...
inline __attribute__((used)) void* __hipstdpar_calloc(std::size_t n, std::size_t sz)
{
  return ::std::memset(__hipstdpar_malloc(n * sz), 0, n * sz);
}

inline __attribute__((used)) int __hipstdpar_posix_aligned_alloc(void** p, std::size_t a, std::size_t n)
...
inline __attribute__((used)) void* __hipstdpar_realloc(void* p, std::size_t n)
{
...
inline __attribute__((used)) void* __hipstdpar_realloc_array(void* p, std::size_t n, std::size_t sz)
{ // TODO: handle overflow in n * sz gracefully, as per spec.
  return __hipstdpar_realloc(p, n * sz);
}

inline __attribute__((used)) void* __hipstdpar_operator_new_aligned(std::size_t n, std::size_t a)
{
...
inline __attribute__((used)) void* __hipstdpar_operator_new_nothrow(std::size_t n, std::nothrow_t) noexcept
{
  try
  {
...
inline __attribute__((used)) void*
...
inline __attribute__((used)) void __hipstdpar_operator_delete_aligned_sized(void* p, std::size_t, std::size_t) noexcept
{
  return __hipstdpar_free(p);
}

inline __attribute__((used)) void __hipstdpar_operator_delete(void* p) noexcept
{
  return __hipstdpar_free(p);
}

inline __attribute__((used)) void __hipstdpar_operator_delete_aligned(void* p, std::size_t) noexcept
{
  return __hipstdpar_free(p);
}

inline __attribute__((used)) void __hipstdpar_operator_delete_sized(void* p, std::size_t n) noexcept
{
...
inline __attribute__((used)) void* __hipstdpar_mmap(void* p, std::size_t n, int prot, int f, int fd, off_t dx) noexcept
{
...
inline __attribute__((used)) int __hipstdpar_munmap(void* p, std::size_t n) noexcept
{
  if (hipstd::__initialised)
  {
    hipDevice_t d{};
...
#    error \
      "__HIPSTDPAR_INTERPOSE_ALLOC_V1__ should be defined. Please use the '--hipstdpar-interpose-alloc' compile option."
